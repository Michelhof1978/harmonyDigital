...
<!-- FIN OFFRES MOBILE -->
    ...
    <!-- FOND BINAIRE -->
    <script>
    const numDots = 100; // Nombre total de points
    ...
    function createDot() {
        ...
        dot.className = 'dot';
        dot.textContent = characters[Math.floor(Math.random() * characters.length)];
        dot.style.left = `${Math.random() * 100}vw`;
        dot.style.top = `${Math.random() * 100}vh`;

        // Définir une animation unique pour chaque point
        const duration = Math.random() * 30 + 25; // Durée entre 25s et 55s
        dot.style.animation = `moveDots ${duration}s linear infinite`;
    ...
    // Initialiser les points lors du chargement de la page
    initializeDots();
    ...
    window.addEventListener('resize', () => {
        ...
        resizeTimeout = setTimeout(initializeDots, 200); // Délai de 200ms avant de réinitialiser
    });
</script>
</body>
</html>
