<?php include("head.php") ?>
<meta name="description" content="Confirmation message - Harmony Digital">
<title>Confirmation message - Harmony Digital</title>
</head>

<?php include("header.php") ?>

<div id="binary-background"></div>

<h1 class="border borderColor border-4 rounded p-2 col m-5 text-center">
        <strong class="text-white">Message Envoyé</strong>
    </h1>

    <section class="confirmation mb-5">

<h2 class="m-5 text-center border border-3 rounded text-white p-2 display-6 h4Index" id="contact"><strong>MESSAGE ENVOYE</strong></h2>

<div class="text-center">
  <div class="lead text-success display-4 text-center p-4 pb-5 bg-white rounded ms-1 me-1 mb-5">
    <p>
      Nous avons bien reçu votre message et nous vous confirmons que votre demande est en cours de traitement.
    </p>
  </div>

</div>

</section>

<?php include("footer.php") ?>

</body>
</html>
